...
static void http_post_task(void *pvParameters)
{
    ...
    while(1) {
        ...
        snprintf(post_request, sizeof(post_request),
                        ...
                        "\r\n"
                        "%s",
                        WEB_PATH, WEB_SERVER, WEB_PORT, strlen(post_data), post_data);

        if (send(s, post_request, strlen(post_request), 0) < 0) {
            ESP_LOGE(TAG, "... socket send failed");
            close(s);
            vTaskDelay(4000 / portTICK_PERIOD_MS);
            continue;
...
void app_main(void)
{
    ...
    xTaskCreate(&http_post_task, "http_get_task", 4096, NULL, 5, NULL);
}
